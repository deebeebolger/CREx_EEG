...
function baselims = CREx_EEGPreprocessing_pipeline_simple(paramfile_path,Trigcfg,coi)
...
DIR_main = fullfile(DIR_main,coi,filesep);
DIR_save = char(Params.DIR_save);
DIR_save = fullfile(DIR_save,coi,filesep);
...
for counter = 21:30 %length(filenum)
    %% OPEN EEGLAB SESSION
    ...
    %% A correction of the channel names for subject 10 DYS
   
%     externals = {'EXG1' 'EXG2' 'EXG3' 'EXG4' 'EXG5' 'EXG6' 'EXG7' 'EXG8'};
%     ext_ind = 65:72;
%     
%     for extcnt = 1:length(externals)
%         EEG.chanlocs(ext_ind(extcnt)).labels = externals{1,extcnt};
%     end
%     
%     [ALLEEG, EEG, CURRENTSET] = eeg_store(ALLEEG, EEG, CURRENTSET);
%     EEG = eeg_checkset( EEG );
...
%     eeglab redraw
    ...
    %% CHECK THAT THE TRIGGERS ARE IN THE CORRECT FORMAT AND CORRECT IF NECESSARY
    
    if length(num2str(EEG.event(10).type)) > 3
        disp('Triggers in incorrect format...need to correct...');
        events_correct = Trigger_Fixit([EEG.event.type]);
        
        for ecnt = 1:length(events_correct)
            EEG.event(ecnt).type = events_correct(ecnt);
        end
    ...
    CREx_SpectCalc_multitap(EEG,65:66,[1 150],[],specnom_ref,.1);
    ...
    CREx_SpectCalc_multitap(EEG,1:64,[1 80],[],specnom_scalp,.1);
    ...
    EEG = JugePhon_trigger_integrate(EEG, Trigcfg,DIRsave_curr);
    
    %EEG = ACE_trigger_integrate(EEG,Tcfg,'Towards');
